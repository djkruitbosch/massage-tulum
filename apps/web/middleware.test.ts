 ...
 * Also mock @supabase/ssr so the middleware can run without real Supabase
 ...
 *
 * Ticket: CU-869d29n0x (FE-3) + CU-869d4za67 (auth middleware extension)
 */
import { NextRequest, NextResponse } from 'next/server';
import { describe, expect, it, vi } from 'vitest';

// Mock @supabase/ssr — always returns no user (unauthenticated baseline for CSP tests)
...
describe('middleware — CSP nonce headers', () => {
  ...
  it('sets an x-nonce header on the response', async () => {
    const response = await middleware(makeRequest('/'));
    expect(response.headers.get('x-nonce')).not.toBeNull();
  });

  it('CSP header contains the nonce value from x-nonce', async () => {
    const response = await middleware(makeRequest('/'));
  ...
  it('CSP header contains default-src self', async () => {
    const response = await middleware(makeRequest('/'));
    expect(response.headers.get('Content-Security-Policy')).toContain(`default-src 'self'`);
  });

  it('CSP header contains strict-dynamic in script-src', async () => {
    const response = await middleware(makeRequest('/'));
    expect(response.headers.get('Content-Security-Policy')).toContain(`'strict-dynamic'`);
  });

  it("script-src does not contain 'unsafe-inline' (security-critical)", async () => {
    const response = await middleware(makeRequest('/'));
    const csp = response.headers.get('Content-Security-Policy')!;
  ...
  it('CSP header contains frame-src none', async () => {
    const response = await middleware(makeRequest('/'));
    expect(response.headers.get('Content-Security-Policy')).toContain(`frame-src 'none'`);
  });

  it('CSP header contains upgrade-insecure-requests', async () => {
    const response = await middleware(makeRequest('/'));
    expect(response.headers.get('Content-Security-Policy')).toContain('upgrade-insecure-requests');
  });

  it('two successive requests produce different nonces (per-request, not module-level)', async () => {
    const response1 = await middleware(makeRequest('/'));
    const response2 = await middleware(makeRequest('/'));
  ...
  it('nonce in x-nonce matches nonce embedded in CSP for each request independently', async () => {
    const r1 = await middleware(makeRequest('/'));
    const r2 = await middleware(makeRequest('/en'));
